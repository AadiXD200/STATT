...
interface SearchOverlayProps {
  ...
  onFilterChange: (filters: { open24h: boolean; pediatric: boolean; parkingAvailable: boolean }) => void;
}

export function SearchOverlay({ hospitals, onClose, onSelect, filters, onFilterChange }: SearchOverlayProps) {
  ...
  const filteredHospitals = hospitals.filter((hospital) => {
    ...
    const matchesFilters =
      (!filters.open24h || hospital.open_24h) &&
      (!filters.pediatric || hospital.pediatric) &&
      (!filters.parkingAvailable || hospital.parking_available);
    ...
    <div className="fixed inset-0 z-50 bg-background/98 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="h-full flex flex-col max-w-3xl mx-auto p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          ...
          <button
            onClick={() => toggleFilter('parkingAvailable')}
            className={`px-4 py-2 font-mono text-xs uppercase tracking-wider transition-colors ${
          ...
          >
            {filters.parkingAvailable ? '[x]' : '[ ]'} Parking Available
          </button>
        </div>
            ...
            {filteredHospitals.map((hospital) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left p-4 hover:bg-surface transition-colors border-l-2 border-transparent hover:border-accent-green"
                >
